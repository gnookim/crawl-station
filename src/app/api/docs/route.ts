...
- 워커 목록 뷰 모드 3종: 리스트 / 🌐 IP 그룹 / 🏷 기능 그룹
- IP 그룹 뷰: 같은 external IP를 공유하는 워커를 하나의 카드로 묶어 표시
  - 온라인 수 / 오프라인 수 / 차단 워커 경고 뱃지 그룹 헤더에 표시
  - IP가 없는 워커는 "미확인" 그룹으로 분리
  - 워커 행에 카테고리 배지 표시 (타입 구분)
- 기능 그룹 뷰: allowed_types 기준으로 네이버 / 인스타그램 / Oclick / 전체 허용 그룹 분류
  - 카테고리별 하루 처리량 합계 (usedNaver/quotaNaver) 그룹 헤더에 표시
  - 한도 소진 시 빨간색 경고
  - 워커 행에 external IP 표시 (같은 IP 위험 식별용)
- 각 그룹 접기/펼치기 가능
- 뷰 모드 선택은 localStorage에 저장 (새로고침 유지)
- 그룹 뷰 내 워커 행에서 수정 패널, 테스트, 재시작/정지/삭제 등 전체 기능 동작
...
### Station — DB 연결 오류 감지 + 에러 UI
- 워커 목록이 비어 보이는 원인: Supabase 프로젝트 일시 정지(522 Connection Timed Out)
- loadData에 try/catch + workersRes.error 체크 추가 — 실패 시 명확한 오류 배너 표시
- DB 오류 시 Supabase 대시보드 링크 + 재시도 버튼 제공
- 로딩 중 스피너 → "워커 목록 로드 중..." 텍스트 추가 (데이터 도착 전 빈 화면 방지)
- React Fragment key 수정: workers.map()에서 <React.Fragment key={w.id}> 사용
...
- 다중 세션 PC: 탭 선택(최신/이전 1/이전 2…)으로 개별 세션 확인 가능
- 세션 상세: 스텝바, 실패 단계, AI 진단, 최근 로그 통합 표시
...
### Station — 워커 관리 인라인 편집 → 수정 패널 통합
- 이름/장소/담당자 인라인 저장 버튼 제거 (저장 저장 저장 반복 제거)
- ⚙설정 버튼 → "수정" 버튼: 우측 슬라이드 패널에서 이름·장소·담당자·메모·네트워크 설정 일괄 편집
- 패널 내 섹션 구분: 기본 정보 / 네트워크·크롤링 / 타입 분류
- 테이블 행: 이름·장소·담당자 정적 표시 (읽기 전용)

### Station — 워커 관리 제어 버튼 레이아웃 수정
- flex-wrap 제거 → flex-nowrap 으로 재시작/정지 줄바꿈 방지
- 버튼 크기 통일 (px-2 py-1 text-xs)

### Station — 사이드바 카테고리 그룹 구조로 재편
- 플랫 목록 → 5그룹 구조: 크롤링 / 설치 / 채널 / 분석 / 시스템
- 일일 순위·순위 대시보드 제거 (비사용 메뉴 정리)
- AI 분석 → 차단 리스크(AI) 명칭 변경
- 축소 모드: 그룹 사이 구분선 표시

### Station — 크롤링 전략 센터 4탭 구조 확장
- 기존 단일 설정 화면 → 4탭 구조: 설정 / 분석 / 차단 분석 / 로그
- **분석 탭**: 오늘 KPI 카드(총 처리·평균 응답·에러율·차단), 24h×카테고리 히트맵, 워커별 성능 테이블, 큐 효율, 타입 분포
- **차단 분석 탭**: 현재 차단 워커 카드, 리스크 스코어(7일 차단×20 + 에러율 + 차단레벨×15), 시간대별 바차트, 플랫폼 분포, 차단 타임라인, AI 자동 조정 이력
- **로그 탭**: 이벤트 스트림(오늘 크롤 메타데이터 실시간 뷰) / 에러 집계(타입별 바차트) / 큐 이력(필터 검색)
- 기존 설정 탭 기능 완전 보존
...
- 워커 백그라운드 실행을 성공/실패 무관하게 항상 처리, done marker 전에 실행
- 워커 검증 실패해도 "설치 완료"로 처리 (실제 크롤링 테스트는 Station에서)
...
- 하단 바 항상 표시, 선택 여부에 따라 컨텍스트 전환:
  - 선택 없음: 전체 N/I/O 테스트 + 업데이트/재시작/정지 버튼
  - 선택 있음: 일괄 네트워크·한도·타입 편집 + 일괄 적용
...
- \`POST /api/releases\` 동일 버전 중복 등록 방지 (upsert 로직)
- naver-crawler \`.github/workflows/auto-release.yml\` 추가
  - VERSION 자동 추출 → Supabase worker_releases 업데이트
  - Mac PKG + Windows 인스톨러 빌드 자동 트리거
...
- 회원 관리 페이지: SSO 직접 호출 → Next.js 프록시 경유로 변경 (브라우저에 SSO URL 노출 제거)

## 2026-04-13

### Station v1.0.0 — 워커 관리 통합 (설정 인라인화)
- /workers 페이지: 체크박스 다중 선택 + 일괄 설정 적용
- /workers 페이지: 워커별 설정 인라인 펼치기 (네트워크/할당량/타입)
- /config 페이지: 워커별 설정 섹션 제거 → 글로벌 크롤링 전략 전용으로 단순화
- 사이드바: "워커 설정" → "크롤링 전략" 명칭 변경
...
### 글로벌 설정 — 새벽 휴식 시간대 UI 추가
- config 페이지에 시간대 선택 버튼 (0~23시, KST) 추가
- 선택한 시간에만 워커 작업 중단 (기본값 3·4·5시)
- 아무것도 선택 안 하면 24시간 운행
...
- Cloudflare Pages (*.pages.dev), sesame11210.com, localhost 허용
